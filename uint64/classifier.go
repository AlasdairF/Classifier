...
func (t *Trainer) DefineCategories(categories []uint64) error {
	...
	t.testDocs = make([][]binsearch.CounterUint64, len(categories))
	t.trainingTokens = make([][]uint64, len(categories))
	return nil
}

// AddTrainingDoc adds a training document to the classifier.
func (t *Trainer) AddTrainingDoc(category []byte, tokens []uint64) error {
...
func (t *Trainer) AddTestDoc(category []byte, tokens []uint64) error {
	// Check to see if category exists already, if it doesn't then add it
	indx, ok := t.categoryIndex.Find(category)
	if !ok {
	...
	t.testDocs[indx] = append(t.testDocs[indx], binsearch.CounterUint64{})
...
func (t *Trainer) ensemble() {
	...
	tokmap := make([]binsearch.CounterUint64, len(t.Categories) * number_of_ensembles)
	ensembleTokAvg := new(binsearch.CounterUint64)
...
func (t *Trainer) Create(allowance float32, maxscore float32) {
	...
	dupfinder := new(binsearch.CounterUint64) // create tally for scores from this category
	...
	for indx, _ := range t.Categories { // loop through categories
		tally := new(binsearch.CounterUint64) // create tally for scores from this category
		for i=0; i<number_of_ensembles; i++ { // loop through ensemble categories
...
func (t *Classifier) Classify(tokens []uint64) []uint64 {
	var tok uint64
	var ok bool
	var i int
	var obj scorer
...
func (t *Classifier) ClassifySimple(tokens []uint64) ([]byte, uint64) {
